...
export default function Products() {
  ...
  const renderStars = (rating) =>
    Array.from({ length: 5 }, (_, i) => {
      if (i + 1 <= Math.floor(rating))
        return <img key={i} src={StarFill} className="star-icon" />;
      if (i + 1 - rating < 1)
        return <img key={i} src={StarHalf} className="star-icon" />;
  ...
  const handleFilterChange = (e) => {
    ...
    if (name === "priceMin")
      setFilters((p) => ({ ...p, price: [Number(value), p.price[1]] }));
    else if (name === "priceMax")
      setFilters((p) => ({ ...p, price: [p.price[0], Number(value)] }));
  ...
  useEffect(() => {
    ...
    result = result.filter(
      (p) => p.newPrice >= filters.price[0] && p.newPrice <= filters.price[1],
    );
    if (filters.rating)
  ...
  const renderCard = (item) => {
    ...
    const cardContent = (
      <div
        className={`deal-card ${outOfStock ? "deal-card--out-of-stock" : ""}`}
      >
  ...
  const seoTitle = searchTerm
    ? `Пошук: ${searchTerm} — Athleon`
    : "Каталог товарів — Athleon";
  const seoDescription = searchTerm
    ? `Результати пошуку "${searchTerm}" в каталозі футбольного екіпірування Athleon.`
    : "Великий вибір футбольного екіпірування: бутси, форма, аксесуари. Доставка Новою Поштою та УкрПоштою по всій Україні.";
  const seoUrl = "https://athelon.netlify.app/products";
